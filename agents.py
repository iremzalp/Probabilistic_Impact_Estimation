...
class Preparation_Agent:
    ...
    def pipeline_usage(self, df, params):
        ...
        # Add weather possibly
        #io add fillna pad for the last na values
        if "temp" in df.columns:
            output["temp"] = df["temp"]
            output["temp"].fillna(method="backfill", inplace=True)
            output["temp"].fillna(method="pad", inplace=True)
        if "dwpt" in df.columns:
            output["dwpt"] = df["dwpt"]
            output["dwpt"].fillna(method="backfill", inplace=True)
            output["dwpt"].fillna(method="pad", inplace=True)
        if "rhum" in df.columns:
            output["rhum"] = df["rhum"]
            output["rhum"].fillna(method="backfill", inplace=True)
            output["rhum"].fillna(method="pad", inplace=True)
        if "wdir" in df.columns:
            output["wdir"] = df["wdir"]
            output["wdir"].fillna(method="backfill", inplace=True)
            output["wdir"].fillna(method="pad", inplace=True)
        if "wspd" in df.columns:
            output["wspd"] = df["wspd"]
            output["wspd"].fillna(method="backfill", inplace=True)
            output["wspd"].fillna(method="pad", inplace=True)
...
class Recommendation_Agent:
    ...
    def cost_by_starting_time(self, date, device, evaluation=False):
        ...
        if not evaluation:
            ...
            device_load = evaluation["load"].loc[date].loc[device]
    ...
    def recommend_by_device(
        ...
        if not evaluation:
            ...
            activity_probs = evaluation["activity"].loc[date]
...
class X_Recommendation_Agent:
    ...
    def recommend_by_device(
        ...
        if not evaluation:
            ...
            # get usage probs
            name = ("usage_" + device.replace(" ", "_").replace("(", "").replace(")", "").lower())
            usage_prob = evaluation[name].loc[date]
...
class Evaluation_Agent:
    ...
    def _get_recommendations(
        ...
        for date in dates:
            try:
                self.output["recommendation"].loc[date] = self.recommendation.pipeline(
    ...
    def _predict_all_activity_usage(self, agent):
        ...
        for date in dates:
            ...
            try:
                ...
                y_hat.loc[date] = self[agent].predict(model, X_test)
            except Exception as e:
                self.errors["evaluation"][agent].loc[date] = type(e).__name__
        return y_hat
    ...
    def _get_cold_start_score(self, agent, fn="default", **kwargs):
        ...
        for date, pred in y_hat.items():
            scores.loc[date] = fn(y_true, pred)
        return scores
...
class Explainability_Agent:
    ...
    def feature_importance(self):
        ...
        self.shap_values = self.explainer_activity.shap_values(
            self.X_test_activity.iloc[self.best_hour, :],check_additivity=False)
    ...
    def explanation_from_feature_importance_activity(self, feature_importance_activity, date, best_hour, diagnostics=False):
        ...
        d = {'features': ['dwpt', 'rhum', 'temp', 'wdir', 'wspd'],
             ...
             'feature_values' : [weather_hourly.loc[date].iloc[best_hour, -5:].loc['dwpt'],
                                 weather_hourly.loc[date].iloc[best_hour, -5:].loc['rhum'],
                                 weather_hourly.loc[date].iloc[best_hour, -5:].loc['temp'],
                                 weather_hourly.loc[date].iloc[best_hour, -5:].loc['wdir'],
                                 weather_hourly.loc[date].iloc[best_hour, -5:].loc['wspd']
                                 ]
    ...
    def explanation_from_feature_importance_usage(self, feature_importance_usage, date, diagnostics=False):
        ...
        d = {'features': ['dwpt', 'rhum', 'temp', 'wdir', 'wspd'],
             ...
             'feature_importances' : [feature_importance_usage.loc[feature_importance_usage[
                                  ...
                                  feature_importance_usage.loc[feature_importance_usage[
                                                                      'col_name'] == 'wspd', 'feature_importance_vals'].to_numpy()[0]],
             'feature_values': [weather_daily.loc.loc[date].loc['dwpt'],
                                weather_daily.loc.loc[date].loc['rhum'],
                                weather_daily.loc.loc[date].loc['temp'],
                                weather_daily.loc.loc[date].loc['wdir'],
                                weather_daily.loc.loc[date].loc['wspd']
             ]
